...
export default function VerifyScreen() {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.post<{ token: string; user: any }>('/auth/verify-otp', {
        email,
        code,
      });
      ...
      Alert.alert('Error', err instanceof Error ? err.message : 'Invalid or expired OTP');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View className="flex-1 justify-center bg-white px-6">
      <View className="mb-8">
        ...
        <Text className="mt-1 text-gray-500">Enter the 6-digit code sent to {email}</Text>
      </View>
      ...
      <TouchableOpacity onPress={handleResend} disabled={resending} className="mt-4">
        <Text className="text-center text-blue-600">{resending ? 'Sending...' : 'Resend OTP'}</Text>
      </TouchableOpacity>
